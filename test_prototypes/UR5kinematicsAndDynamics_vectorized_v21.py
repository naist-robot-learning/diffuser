...
def compute_analytical_jacobian(q):     
    ...
    if timer:
        ...
        start_time = time.time()
        jac = torch.autograd.functional.jacobian(__fkine__, q, retain_graph=True, create_graph=False, vectorize=True).to('cuda')
        #jac = jacrev(__fkine__)(q).to('cuda')
        end_time = time.time()
...
def compute_inertia_matrix(q):
    ...
    Mq[:,0,2] = Mq[:,2,0] = 0.01615783641*cos_q234 + 0.006888811168*sin_q235 - 0.0004508000672*cos_q234_2q5 + 0.006888811168*sin_q23neg5 + 0.00352803026*cos_q5neg3neg4neg2 + 0.0004508000672*cos_2q5_neg342 - 0.0002987944852*cos_q2345 + 0.1313181732*sin_q23 #m13 m31
    Mq[:,0,1] = Mq[:,1,0] = Mq[:,0,2] + 0.007460552829*sin_q25 + 0.007460552829*sin_q2_neg5+ 0.348513447*sin_q2 #m12 m21
    Mq[:,0,3] = Mq[:,3,0] = 0.01615783641*cos_q234 - 0.0004508000672*cos_q234_2q5 + 0.00352803026*cos_q5neg3neg4neg2 + 0.0004508000672*cos_2q5_neg342 - 0.0002987944852*cos_q2345 #m14 m41
    Mq[:,0,4] = Mq[:,4,0] = 0.006888811168*sin_q23neg5 - 0.006888811168*sin_q235 - 0.002840479501*cos_q234 - 0.0002987944852*cos_q5neg3neg4neg2 - 0.00352803026*cos_q2345 - 0.007460552829*sin_q25 + 0.007460552829*sin_q2_neg5 #m15 m51
    Mq[:,0,5] = Mq[:,5,0] = -0.000138534912*sin_q234*sin_q5 #m16 m61
    Mq[:,1,2] = Mq[:,2,1] = 0.7014282607*cos_q3 - 0.1166034794*sin_q4 - 0.063140533*cos_q3*sin_q4 - 0.063140533*cos_q4*sin_q3 + 0.02755524467*cos_q4*sin_q5 - 0.001803200269*cos_q5**2 - 0.01492110566*sin_q3*sin_q4*sin_q5 + 0.01492110566*cos_q3*cos_q4*sin_q5 + 0.644094696 #m23 m32   
    Mq[:,1,3] = Mq[:,3,1] = 0.01377762234*cos_q4*sin_q5 - 0.063140533*cos_q3*sin_q4 - 0.063140533*cos_q4*sin_q3 - 0.05830173968*sin_q4 - 0.001803200269*cos_q5**2 - 0.01492110566*sin_q3*sin_q4*sin_q5 + 0.01492110566*cos_q3*cos_q4*sin_q5 + 0.01612863983 #m24 m42
    ...
    Mq[:,2,3] = Mq[:,3,2] = 0.01377762234*cos_q4*sin_q5 - 0.05830173968*sin_q4 - 0.001803200269*cos_q5**2 + 0.01612863983 # m34 m43
...
def compute_reflected_mass(q, u):
    ...
    print("Jacobian computation time: ", end - start)
    #import ipdb; ipdb.set_trace()
    start = time.time()
    Mq = compute_inertia_matrix(q)
    end = time.time()
    print("Inertia matrix computation time: ", end - start)
    start = time.time()
    J_T = J.permute(0,2,1)
    M_x_inv = (J @ torch.linalg.solve(Mq, J_T))[:,:3,:3]
    u_T = u.permute(0,2,1)
    end = time.time()
    print("Matrix inversion time: ", end - start)
    start = time.time()
    mu = 1/(torch.matmul(u_T, torch.matmul(M_x_inv, u)).squeeze())
    mu = einops.rearrange(mu, '(b h) -> b h', b=b, h=h)
    end = time.time()
    print("Reflected mass computation time: ", end - start)
    return mu