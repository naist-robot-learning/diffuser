...
def run_cost_function_exec_time_test_vectorized_precompiled():
    ...
    u[:,0] = 1; u[:,1] = 0; u[:,2] = 0
    for i in range(0,batch_size):
        cost[i,:] = model.compute_precompiled_reflected_mass_with_precompiled_jacobian(x[i,6:12,:], u)
...
def run_cost_function_exec_time_test_vectorized_all_batched():
    ...
    for i in range(0, 2):
        torch.manual_seed(42)
        x = torch.randn(8,4,26).to("cuda").requires_grad_()
        ####### (TODO) VECTORISED IMPLEMENTATION ######## 
        start = torch.cuda.Event(enable_timing=True)
        end = torch.cuda.Event(enable_timing=True)
        ...
        #torch.cuda.synchronize()
        print("final_cost", final_cost)
        print("Cost computation time: ", end - start)
        start = time.time()
        grad = torch.autograd.grad([final_cost.sum()], [x])[0]
        end = time.time()
        torch.cuda.synchronize()
        print("Gradient computation time: ", end - start)
    import ipdb; ipdb.set_trace()
    return final_cost
