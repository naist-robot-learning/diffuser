...
class UR5Model:
    def __init__(self):
        self.DH_params = self._fill_in_DH_table()
    ...
    def _fill_in_DH_table(self):
        """ Fills in the standard DH table with the UR5 specific parameters
        
        Returns:
            DH_params (6x4 torch tensor): UR5 DH parameters
        ...
        DH_params = torch.empty((6,3), dtype=torch.float32).to('cuda')
        DH_params[0,:] = torch.tensor([0.0, torch.pi/2, 0.08946])
        DH_params[1,:] = torch.tensor([-0.425, 0.0, 0.0])
        DH_params[2,:] = torch.tensor([-0.39225, 0.0, 0.0])
        DH_params[3,:] = torch.tensor([0.0, torch.pi/2, 0.10915])
        DH_params[4,:] = torch.tensor([0.0, -torch.pi/2, 0.09465])
        DH_params[5,:] = torch.tensor([0.0, 0.0, 0.0823])
