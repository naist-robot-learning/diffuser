...
def dh_to_transformation(a, alpha, d, theta):
    ...
    T = torch.eye(4, dtype=torch.float32, device='cuda').unsqueeze(0).expand(batch_size, -1, -1).clone()
    T[:,0,0] = cos_theta
...
def compute_fk(q):
    ...
    T = torch.eye(4, dtype=torch.float32, device='cuda').unsqueeze(0).expand(batch_size, -1, -1)
...
def hom_to_pose(H):
    ...
    """
    batch_size = H.shape[0]
    pose = torch.empty((batch_size, 6), dtype=torch.float32).to('cuda')
    pose[:,0] = H[:,0, 3]
    pose[:,1] = H[:,1, 3]
    pose[:,2] = H[:,2, 3]
...
def compute_analytical_jacobian(q):     
    batch_size = q.shape[1]
    jac = torch.autograd.functional.jacobian(fkine, q, create_graph=False).to('cuda')
    J = torch.empty((batch_size,6, 6), dtype=torch.float32).to('cuda')
...
def compute_inertia_matrix(q):
    ...
    batch_size = q.shape[1]
    Mq = torch.empty(batch_size,6, 6, dtype=torch.float32).to('cuda')
    q1 = q[0,:]
    q2 = q[1,:]
    q3 = q[2,:]
    q4 = q[3,:]
