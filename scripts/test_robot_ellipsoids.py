...
def update_plot(frame, line, line2, scatter, quiver):
    ...
    # Update plot
    line.set_data_3d(
        [p[0] for p in new_positions[:-1]], [p[1] for p in new_positions[:-1]], [p[2] for p in new_positions[:-1]]
    )
    line2.set_data_3d(
        [p[0] for p in new_positions[-2:]], [p[1] for p in new_positions[-2:]], [p[2] for p in new_positions[-2:]]
    )
    scatter._offsets3d = (
...
# Directory containing .npz files
directory = "/home/ws/src/diffuser/Experiments/C10_C14_diffuser/"  # Current directory
...
ani.save(f"{npz_file_name}_animation_{traj}_presentation_diffuser_none.gif", writer="pillow", fps=30)
