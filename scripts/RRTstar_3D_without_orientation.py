...
# mesh_paths = ["/path/to/meshes"]
# end_effector_name = "end_effector"

# # Create the class instance
# reflected_mass_gradient = ReflectedMassGradient(urdf_path, mesh_paths, end_effector_name)

# # Example joint configuration and direction vector
# q_example = np.random.rand(reflected_mass_gradient.n_joints)
# u_example = np.array([1, 0, 0])

# # Evaluate the gradient at the example joint configuration and direction vector
# gradient_value = reflected_mass_gradient.compute_gradient(q_example, u_example)

# print("Gradient of the reflected mass:\n", gradient_value)
...
class RRTStar:
    ...
    def steer(self, from_node, to_pose):
        ...
        # theta = d[-1]
        # new_theta = theta + self.step_size_ori
        # u = d[3:6]  # u_x, u_y, u_z
        # u_sin = np.sin(new_theta / 2) * u
        # w = np.array([np.cos(new_theta / 2)])
        # new_orientation = np.concatenate([u_sin, w])  # x,y,z, w
...
def run_experiment(exp_id: str, num_trials: float, robot_type: str, gain: float, verbose=True):
    ...
    for i in range(N_EXPERIMENTS):
        ...
        # print("Trajectory Configurations:")
        # print(trajectory_configs)
        # norms = np.linalg.norm(trajectory_configs, axis=1)
        # print("norms: ", np.linalg.norm(trajectory_configs, axis=1))
        # print("average_reflected mass")
        # import ipdb
...
if __name__ == "__main__":
    someDictionary = run_experiment(1, 10, "kuka", 1e-1, verbose=False)
    import ipdb
