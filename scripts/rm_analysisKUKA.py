...
def forward_kinematics(theta):
    ...
    DH_params = np.array(
        [
            [0.0, np.pi / 2, 0.3105],  # link 1 parameters 0.3105
            [0.0, -np.pi / 2, 0.0],  # link 2 parameters
            [0.0, -np.pi / 2, 0.4],  # link 3 parameters
            [0.0, np.pi / 2, 0.0],  # link 4 parameters
            [0.0, np.pi / 2, 0.39],  # link 5 parameters
            [0.0, -np.pi / 2, 0.00],  # link 6 parameters
            [0.0, 0.0, 0.0933],
        ]
    )
    T = np.eye(4)
