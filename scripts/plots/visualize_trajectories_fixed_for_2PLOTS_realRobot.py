...
def visualize_trajectories(directory):
    ...
    for npz_file in npz_files:
        ...
        config.append(npz_file)
        all_data.append(q)

        # Debugging: check the first few files if necessary
        if cnt < 8 and len(npz_files) > 1:
            cnt += 1
            continue
        ...
        title = f"Path\ Up \u2192 Middle: RM {metric}imization along x-axis"
        title = f"RM minimization along x-axis"
        ax.set_title(title, fontsize=12, y=0.95)
        filename = f"/home/ws/src/diffuser/Experiments/{exp_name}_results"
    ...
    # Set the format of ticks to two decimal places for both axes
    print("file order: ", config)
