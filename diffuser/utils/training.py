...
def get_dataset(
    ...
    if train_size != int(train_size):
        train_size = int(train_size + 1)
...
class Trainer(object):
    def __init__(
        ...
        self.step = 0
        self.writer = SummaryWriter(log_dir=results_folder + "/tensorboard")
    ...
    def render_samples(self, batch_size=2, n_samples=2):
        ...
        for i in range(batch_size):
            ...
            normed_observations = np.concatenate(
                [np.repeat(normed_conditions, n_samples, axis=0), normed_observations], axis=1
            )
